...
#include <conio.h>
#include <fstream>
//----------------------PROBLEM IS IN BREAKOUTS RED DOTS CHECK IT--------------
using namespace std;
// CONST VARIABLES
...
int main_menu();
//------------------------FOR FILE HANDLING---------------------------
void loadcustomer();                              // usedto load customers with pins from file
void savecustomerInFile(string name, string pin); // used to store data back to file

// used for customer billing section
void loadcustomer_bills();
void save_customer_n_bill_infile(string name, int bill);

// used for adding stock in file
void loadstockandprice();
void savestockInFile();

// used for adding shop bills in file
// void loadbills_shop();
// void saveshopbillsInFile(string name, int prices);
//------------------------FOR ADMIN SECTION---------------------------
int admin_menu();
void add_customer();
...
void add_stock_in_array(string name, int price);
void view_stock();
void update_stock();
int shop_management_menu();
void add_bills_shop();
void add_shopbills_array(string name, int amount);
            ...
            while (true)
            {
                ...
                else if (admin_option == 5) // Add / Update stock of the shop
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            if (stock_prices[0] != 0 && stock_adding[0] != " ")
                            {
                                view_stock();
                                clear_screen();
                            }
...
{
    fstream file;
    file.open("names_pins.txt", ios::out);
    file << "\n"
         << name << " " << pin;
...
void add_customer()
{
    ...
    {
        cout << "Only " << N_CUSTOMER << " can be added " << endl;
    }
}
void add_customer_array(string name, string pin)
{
...
{
    fstream file;
    file.open("stocks_of_shop.txt", ios::in);
    string name;
    int price;
    // stock_count = 0;
    while (!file.eof())
    {
        file >> name;
        file >> price;
        add_stock_in_array(name, price);
    }
    file.close();
}
void add_stock_in_array(string name, int price)
{
    stock_adding[stock_count] = name;
    stock_prices[stock_count] = price;
    stock_count++;
}
void savestockInFile()
{
    ...
    int index = 0;
    file.open("names_pins.txt", ios::out);
    while (index < stock_count)
    {
        file << "\n"
             << stock_adding[index] << " " << stock_prices[index];
        index++;
    }
    // cout << "Record saved into file " << endl;
    file.close();
...
void adding_stock_with_price()
{
    if (stock_count < 10)
    {
        ...
        for (int i = 0; i < n_stock; i++)
        {
            ...
            cin >> prices;
            add_stock_in_array(name, prices);
        }
        cout << "You stocks are being added." << endl;
    }
    else
    {
        cout << "You can only add " << N_STOCKS << " stocks" << endl;
    }
}
...
void view_stock()
{
    ...
    {
        cout << stock_adding[i] << "\t\t" << stock_prices[i] << endl;
    }
}
...
{
    fstream file;
    file.open("bills_of_shop.txt", ios::in);
    string name;
    int amount;
    shop_bill_count = 0;
    while (!file.eof())
    {
        file >> name;
        file >> amount;
        add_stock_in_array(name, amount);
    }
    file.close();
}
void add_shopbills_array(string name, int amount)
{
    ...
    bills_shop[shop_bill_count] = amount;
    shop_bill_count++;
}
void saveshopbillsInFile(string name, int prices)
{
    fstream file;
    file.open("bills_of_shop.txt", ios::app);
    file << "\n"
         << name << " " << prices << endl;
    // cout << "Record saved into file " << endl;
    file.close();
...
void add_bills_shop()
{
    if (shop_bill_count < N_BILLS)
    {
        ...
        for (int i = 0; i < N_BILLS; i++)
        {
            ...
            cin >> bill_name;
            cout << "Enter " << bill_name << " bill amount : ";
            cin >> bill_amount;
            add_shopbills_array(bill_name, bill_amount);
            saveshopbillsInFile(bill_name, bill_amount);
        }
        cout << "Your bills added" << endl;
    }
    else
    {
        cout << "You can only add " << N_BILLS << " types of bills" << endl;
    }
}
void sorting(int list[], string names[], int array_size)
{
...
void bill_generator()
{
    ...
    if (customer_n_a[bill_count] == customer_n && customer_p_a[bill_count] == customer_p)
    {
        ...
        save_customer_n_bill_infile(customer_n, t_bill);
...
{
    fstream file;
    file.open("names_bills.txt", ios::in);
    string name;
    int bill;
    bill_count = 0;
    while (!file.eof())
...
{
    fstream file;
    file.open("names_bills.txt", ios::app);
    file << "\n"
         << name << " " << bill << endl;
    cout << "Record saved into file " << endl;
    file.close();
